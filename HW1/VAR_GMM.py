...
class GMM_for_VAR(GMM):
    ...
    def __init__(self, *args, **kwds):
        ...
        super(GMM_for_VAR, self).__init__(*args, **kwds)

    def momcond(self, params):
        y_sample = self.endog
        z = self.exog
        if len(params.shape) == 1:
            g = np.dot(z.T, (y_sample - np.dot(z, params.reshape(int(len(params)/self.k_params), self.k_params))))
        else:
            g = np.dot(z.T, (y_sample - np.dot(z, params)))
        return g
...
class VAR(TimeSeriesModel):
    ...
    def fit(self, maxlags=None, method='ols', ic=None, trend='c',
        ...
        self.data.cov_names = pd.MultiIndex.from_product((self.data.xnames,
                                                          self.data.ynames))
        return self._estimate_var(lags, method=method, trend=trend)

    def _estimate_var(self, lags, offset=0, method='ols', trend='c'):
        """
        lags : int
        ...
        if method == 'gmm':
            k_moms = params.shape[1]
            k_params = params.shape[1]
            params_start = params.reshape(params.shape[0], k_params)
            gmm_model = GMM_for_VAR(y_sample, z, None, k_moms=k_moms, k_params=k_params)

            #try:
            res_gmm = gmm_model.fit(params_start, maxiter=1000, optim_method='nm')

            #except np.linalg.LinAlgError:
            #    res_gmm = gmm_model.fit(params_start, maxiter='cue', optim_method='nm', wargs=dict(centered=False))

            params = res_gmm.params.reshape(int(len(res_gmm.params)/k_params),k_params)
